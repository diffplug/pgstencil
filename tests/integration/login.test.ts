...
appTest(
  'code login snapshots and fixed 24-hour session expiry',
  async ({ f }) => {
...
appTest(
  'link preview does not consume; only requesting browser can confirm',
  async ({ f }) => {
...
appTest(
  'wrong codes commit attempt counts and exhaust the challenge',
  async ({ f }) => {
    const flow = await begin(f);
    for (let i = 0; i < 5; i++)
      await post(
...
appTest.for(['code', 'link'] as const)(
...
appTest(
  'concurrent code and link redemption produces exactly one session',
  async ({ f }) => {
    const flow = await begin(f);
...
appTest(
  'resend cooldown and replacement invalidate old secrets',
  async ({ f }) => {
    const flow = await begin(f);
    await post(
      f,
...
appTest('logout revokes replay and rejects missing CSRF', async ({ f }) => {
  const flow = await login(f);
...
appTest(
  'origin, CSRF and browser binding reject forged requests',
  async ({ f }) => {
    const flow = await begin(f);
    await f.client
      .post('/login/code')
...
appTest(
  'a returning user gets a fresh session and the previous token is revoked',
  async ({ f }) => {
...
test('send limits aggregate across browsers and server instances', async ({
  ...
  const b = await secondApp(a, 'instance-b');
